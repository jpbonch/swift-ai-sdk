...
`generateText` gives you the finished result; `streamText` hands you every
event as it happens. Both drive the same loop underneath, and the rest of the
docs are variations on this page.

If the loop itself is new to you, read [the loop](/docs/foundations/the-loop)
first. It covers what a step is and why there's a limit on how many the run may
take.
